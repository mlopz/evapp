...
module.exports = async function rebuildConnectorSessions(pool, options = {}) {
  ...
  while (offset < totalEvents) {
    ...
    function isValidTimestamp(ts) {
      const minEpoch = 1420070400; // 2015-01-01 en segundos
      if (typeof ts === 'number' && ts > minEpoch) return true;
      if (typeof ts === 'string' && !isNaN(Date.parse(ts)) && new Date(ts).getFullYear() > 2015) return true;
      return false;
    }
