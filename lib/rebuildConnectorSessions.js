...
function toISODateSafe(ts) {
  ...
  pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
    `[REBUILD-DEBUG] Timestamp original: ${ts}, Fecha generada: ${date ? date.toISOString() : 'null'}`
  ]).catch(() => {});
  if (!date || isNaN(date.getTime()) || date.getFullYear() < 2015) {
    return null;
  }
