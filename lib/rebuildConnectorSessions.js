...
module.exports = async function rebuildConnectorSessions(pool, options = {}) {
    ...
    if (event.status === 'Charging') {
      if (!lastSession[key]) {
        ...
        await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
          `[REBUILD-FIX] Apertura sesión: ${event.charger_name} - ${event.connector_id} | status: ${event.status} | ts: ${event.timestamp}`
        ]);
      } else {
        await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
          `[REBUILD-FIX] Ignorado Charging repetido: ${event.charger_name} - ${event.connector_id} | ts: ${event.timestamp}`
        ]);
      }
    } else {
      if (lastSession[key]) {
        ...
        await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
          `[REBUILD-FIX] Cierre sesión: ${event.charger_name} - ${event.connector_id} | status cierre: ${event.status} | ts cierre: ${event.timestamp} | ts apertura: ${lastSession[key].session_start} | duración: ${duration_minutes} min`
        ]);
        lastSessionEnd[key] = session_end;
        lastSession[key] = null;
      } else {
        await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
          `[REBUILD-FIX] Evento de cierre sin sesión abierta: ${event.charger_name} - ${event.connector_id} | status: ${event.status} | ts: ${event.timestamp}`
        ]);
      }
    }
  }
  ...
  for (const key in lastSession) {
    ...
    if (sesion) {
      ...
      await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
        `[REBUILD-FIX] Cierre artificial (70min) para ${sesion.charger_name} - ${sesion.connector_id} | ts apertura: ${sesion.session_start} | ts cierre: ${session_end.toISOString()} | duración: ${duration_minutes} min`
      ]);
    }
  }
