...
module.exports = async function rebuildConnectorSessions(pool, options = {}) {
  ...
  while (offset < totalEvents) {
    ...
    for (const key in lastSession) {
      ...
      if (sesion) {
        // Buscar el último evento Charging para este conector en el lote
        const eventosConector = events.filter(e =>
          e.charger_name === sesion.charger_name &&
          e.connector_id === sesion.connector_id &&
          e.status === 'Charging'
        );
        const ultimoEvento = eventosConector[eventosConector.length - 1];
        const session_start = new Date(sesion.session_start);
        const session_end = ultimoEvento ? new Date(toISODateSafe(ultimoEvento.timestamp)) : new Date(session_start.getTime() + 70 * 60000);
        const duration_minutes = Math.round((session_end - session_start) / 60000);
        sessionsToInsert.push({
