...
export default function ChargerConnectorsView({ charger, onSelectConnector, sessions = [] }) {
  ...
  function getConnectorStats(connectorId) {
    const filtered = sessions.filter(s => s.chargerName === charger.name && s.connectorId === connectorId && s.start && (s.end || s.end === null));
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {charger.connectors.map((conn) => {
          const stats = getConnectorStats(conn.connectorId);
          return (
            <button
            ...
            >
              ...
              <span className="text-xs text-gray-500">ID: <b>{conn.connectorId}</b></span>
            </button>
          );
        })}
      </div>
