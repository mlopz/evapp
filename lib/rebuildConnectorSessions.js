...
module.exports = async function rebuildConnectorSessions(pool, options = {}) {
  ...
  // Obtener todos los eventos de todos los cargadores (se elimina el filtro por 'Auxicar')
  ...
  // --- DIAGNÓSTICO: Logging detallado de inserciones ---
  await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
    `[REBUILD-DEBUG] Comenzando a insertar ${sessionsToInsert.length} sesiones.`
  ]);
  ...
  for (const s of sessionsToInsert) {
    try {
      ...
      await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
        `[REBUILD-DEBUG] Sesión insertada: ${s.charger_name} - ${s.connector_id} (${s.session_start} a ${s.session_end})`
      ]);
    } catch (err) {
      await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
        `[REBUILD-ERROR] Fallo al insertar sesión: ${s.charger_name} - ${s.connector_id} | Error: ${err.message}`
      ]);
    }
  }

  await pool.query('INSERT INTO rebuild_debug_logs (log) VALUES ($1)', [
    `[REBUILD-DEBUG] Fin de inserción de sesiones.`
  ]);
